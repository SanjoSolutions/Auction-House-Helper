local function IsGear(itemID)
  ...
  return AuctionHouseHelper.Utilities.IsEquipment(classType)
end

function AuctionHouseHelper.Utilities.DBKeyFromBrowseResult(result)
  if result.itemKey.battlePetSpeciesID ~= 0 then
    return {"p:" .. tostring(result.itemKey.battlePetSpeciesID)}
  elseif IsGear(result.itemKey.itemID) and result.itemKey.itemLevel >= AuctionHouseHelper.Constants.ITEM_LEVEL_THRESHOLD then
    return {
